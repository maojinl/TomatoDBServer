...
//	BOOL bRet = ConnectWorldServer( ) ;
	//BOOL	bRet = ConnectBillingServer();
	m_IsConnectedWorld = 0 ;
...
__ENTER_FUNCTION
	...
	if( m_IsConnectedWorld && !IsWorldServerActive())
	{
		Log::SaveLog( SERVER_LOGFILE, "ERROR: World Disconnected, Need restart the World server!" ) ;
		MySleep(2000) ;
	}
	else
	{
