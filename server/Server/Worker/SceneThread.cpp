...
__ENTER_FUNCTION
	...
	_MY_TRY
	{
		...
		for( i=0; i<m_nSceneCount; i++ )
		{
			Log::SaveLog( SERVER_LOGFILE, "SceneThread::run() TID:%d SID:%d SName:%d...", 
				cTid, m_apScene[i]->SceneID(), g_pWorkerManager->GetWorkerInfo(m_apScene[i]->SceneID())->m_ThreadIndex ) ;
		}
...
__ENTER_FUNCTION
	...
	if( m_nSceneCount>=MAX_WORKER_PER_THREAD )
	{
		Assert(FALSE) ;
		return FALSE ;
	}
