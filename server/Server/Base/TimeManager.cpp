...
__ENTER_FUNCTION
	...
	/*TM->tm_year = (Date>>26)&0xf ;	该方法不可取 32位足以存放 timestamp
	TM->tm_mon  = (Date>>22)&0xf ;		timestamp中有众多的数据,不是用几位就可以描述的
	TM->tm_mday = (Date>>17)&0x1f ;
	TM->tm_hour = (Date>>12)&0x1f ;
	TM->tm_min  = (Date>>6) &0x3f ;
	TM->tm_sec  = (Date)    &0x3f ;*/

	time_t	timeType = (time_t)Date;
	tm		*tempTm=localtime(&timeType);
	memcpy(TM,tempTm,sizeof(tm));
...
LONGLONG TimeManager::GetAccurateTime()
{
#ifdef	__WINDOWS__
	...
	{
		//系统支持
		return performanceCounter.QuadPart-m_StartTickCount.QuadPart;
	}
	else
	{
		//系统不支持
...
LONGLONG TimeManager::GetCpuFrequency()
{
#ifdef	__WINDOWS__
	return m_CpuFrequency.QuadPart;
#else
	//Linux不是用这个实现
	return 0;
#endif
...
UINT TimeManager::DiffTimeSec( tm *tm1,tm *tm2 )
{
	__ENTER_FUNCTION
...
BOOL TimeManager::IsYesterday(tm *today,tm *yesterday )
{
	if (today->tm_year == yesterday->tm_year)
	{
		//同一年,直接减
		return (today->tm_yday - yesterday->tm_yday) == 1;
	}
	else
	{
		//非同一年
		if(yesterday->tm_year - today->tm_year == 1)
		{
			//确实是上一年
			if (yesterday->tm_yday == 365
				&&today->tm_yday == 0)
			{
				return TRUE;
			}
