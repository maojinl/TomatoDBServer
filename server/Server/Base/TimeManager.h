...
public :
	...
	//取得两个tm日期之前时间差(单位:秒)   tm2 - tm1
	UINT			DiffTimeSec(tm	*tm1,tm  *tm2);
	//将一个UINT的日期转换成一个tm结构
	...
	BOOL			IsYesterday(tm	*today,tm	*yesterday);

	LONGLONG		GetAccurateTime();

	LONGLONG		GetCpuFrequency();
...
public :
	...
	tm				m_TM ;
#ifdef __WINDOWS__
	LARGE_INTEGER	m_StartTickCount;
