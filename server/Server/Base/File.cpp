...
BOOL File::Open( const CHAR* filename, INT flags )
{
	...
	m_fd = EnvFileAPI::open_ex( filename, flags ) ;
	if( m_fd < 0 )
	{
		m_fd = INVALID_HANDLE ;
		return FALSE ;
...
BOOL File::Open( const CHAR* filename, INT flags, INT mode )
{
	...
	strncpy( m_szFileName, filename, _MAX_PATH-1 ) ;
        m_fd = EnvFileAPI::open_ex(filename, flags, mode);
	if( m_fd < 0 )
	{
		return FALSE ;
	}
...
UINT File::Read( VOID* buf, UINT len )
{
	...
	return EnvFileAPI::read_ex(m_fd, buf, len);
...
UINT File::Write( const VOID* buf, UINT len )
{
	...
	return EnvFileAPI::write_ex(m_fd, buf, len);
...
long File::Seek( long offset, INT whence )
{
	...
	return EnvFileAPI::lseek_ex(m_fd, offset, whence);
...
long File::Tell( )
{
	...
	return EnvFileAPI::tell_ex(m_fd);
