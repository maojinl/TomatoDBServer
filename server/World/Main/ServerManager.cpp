...
__ENTER_FUNCTION
	...
	while( IsActive() )
	{
		...
		_MY_TRY
		{
			ret = ProcessTimerCommands( uTime ) ;
			Assert( ret ) ;
		}
		_MY_CATCH
		{
...
BOOL ServerManager::ProcessTimerCommands(UINT uTime )
{
__ENTER_FUNCTION

	BOOL ret = FALSE ;
	UINT iQueSize = m_PacketQue.size();
	PACKET_QUE_ITER Iter;


	for( Iter = m_PacketQue.begin(); Iter != m_PacketQue.end() ; )
	{
		Packet* pPacket = NULL ;
		PlayerID_t PlayerID ;
		UINT Flag ;

		ret = RecvPacket( Iter, pPacket, PlayerID, Flag ) ;
		if( !ret )
			break ;
		...
		if( Flag==PF_REMOVE )
		{
			Iter = RemovePacket(Iter);
			continue ;
		}

		UINT tProcessTime = pPacket->GetProcessTime();
		if(tProcessTime > uTime) 
		{//未到执行时间
			++Iter;
			continue ;		
		}
...
BOOL ServerManager::RecvPacket( PACKET_QUE_ITER Iter, Packet*& pPacket, PlayerID_t& PlayerID, UINT& Flag )
{
__ENTER_FUNCTION
	TIMER_PACKET*  PacketInfo = *Iter;
	pPacket		= PacketInfo->m_pPacket;
	PlayerID	= PacketInfo->m_PlayerID;
	Flag        = PacketInfo->m_Flag;
...
PACKET_QUE_ITER ServerManager::RemovePacket(PACKET_QUE_ITER Iter)
{
__ENTER_FUNCTION
	if( Iter ==  m_PacketQue.end())	return Iter;
	TIMER_PACKET* PacketInfo = *Iter;	
	
	SAFE_DELETE(PacketInfo);

	return m_PacketQue.erase(Iter);
...
BOOL ServerManager::SendTimerPacket( Packet* pPacket, PlayerID_t PlayerID, UINT Flag )
{
__ENTER_FUNCTION
	AutoLock_T autolock(m_Lock);
	TIMER_PACKET *PacketQue = new TIMER_PACKET();;

	PacketQue->m_pPacket = pPacket ;
	PacketQue->m_PlayerID = PlayerID ;
	PacketQue->m_Flag = Flag ;

	m_PacketQue.push_back(PacketQue);
