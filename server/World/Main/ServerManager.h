...
struct TIMER_PACKET
{
	...
	~TIMER_PACKET( )
	{
		g_pPacketFactoryManager->RemovePacket( m_pPacket ) ;
...
typedef	vector<TIMER_PACKET*>	PACKET_QUE ;  
typedef	PACKET_QUE::iterator	PACKET_QUE_ITER ;  
...
public :
	...
	//处理定时消息
	BOOL				ProcessTimerCommands( UINT uTime );
	//新连接接收处理
...
public :
	...
	BOOL				SendTimerPacket( Packet* pPacket, 
									PlayerID_t PlayerID, 
									UINT Flag=PF_NONE ) ;
...
private :
	...
	PACKET_QUE m_PacketQue;

	BOOL RecvPacket( PACKET_QUE_ITER Iter, Packet*& pPacket, PlayerID_t& PlayerID, UINT& Flag );
	PACKET_QUE_ITER RemovePacket( PACKET_QUE::iterator Iter);
