...
UINT CalculateDir( FLOAT fDir )
{
			FLOAT a = 2*__PI/16;

			if ( fDir> a && fDir <= 3*a)
				return 7;
			else if ( fDir> 3*a && fDir <= 5*a)
				return 6;
			else if ( fDir> 5*a && fDir <= 7*a)
				return 5;
			else if ( fDir> 7*a && fDir <= 9*a)
				return 4;
			else if ( fDir> 9*a && fDir <= 11*a)
				return 3;
			else if ( fDir> 11*a && fDir <= 13*a)
				return 2;
			else if ( fDir> 13*a && fDir <= 15*a)
				return 1;
			else if ( fDir <= a || fDir > 15*a)
				return 8;
			else
				return 1;
}
...
//×Ö·û´®È«Ìæ»»
INT StringReplace(string &mainStr, string srcStr, string dstStr)
{
	INT replacedNum = 0;
	for (string::size_type pos(0); pos != string::npos; pos += 1)
	{
		pos = mainStr.find(srcStr, pos);
		if (pos != string::npos)
		{
			mainStr.replace(pos, 1, dstStr);
			++replacedNum;
		}
		else
		{
...
//×Ö·û´®Ìæ»»£¬Ö¸¶¨Ìæ»»¸öÊý
INT StringReplace(string &mainStr, string srcStr, string dstStr, INT maxReplaceNum)
{
	if (maxReplaceNum < 0)
	{
		return StringReplace(mainStr, srcStr, dstStr);
	}

	INT replacedNum = 0;
	for (string::size_type pos(0); pos != string::npos; pos += 1)
	{
		if (replacedNum < maxReplaceNum)
		{
			pos = mainStr.find(srcStr, pos);
			if (pos != string::npos)
			{
