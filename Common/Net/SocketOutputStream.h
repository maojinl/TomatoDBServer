...
//初始化的发送缓存长度
#define DEFAULTSOCKETOUTPUTBUFFERSIZE 96*1024
//最大可以允许的缓存长度，如果超过此数值，则断开连接
#define DISCONNECTSOCKETOUTPUTSIZE 128*1024
...
public :
	//UINT						Write( const CHAR* buf, UINT len ,TYPE_INDEX TypeIndex = T_STRING) ;
	//UINT						Write_ex( const CHAR* buf, UINT len ) ;
	UINT						Write( const CHAR* buf, UINT len ) ;
	//UINT						AddListNode( UINT len ) ;
